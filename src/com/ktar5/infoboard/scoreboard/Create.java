...
public class Create {
    ...
    public static boolean createScoreBoard(Player player) {
        ...
        if (WorldGuard.boardsAllowedHere(player.getLocation())
                && !Settings.isWorldDisabled(player.getWorld().getName())
                && player.hasPermission("InfoBoard.View")
...
//                .getName().equalsIgnoreCase("InfoBoard"))) {
            ...
            if (Settings.doesWorldHaveScoreBoard(InfoBoard.getTimers()
                    .getPage(), player.getWorld().getName()))
                worldName = player.getWorld().getName();
            else if (Settings.doesGlobalHaveScoreBoard(InfoBoard.getTimers()
                    .getPage()))
                worldName = "global";
            else
                return false;
            ...
            // player's rankName
            if (Settings.doesRankHaveScoreBoard(
                    InfoBoard.getTimers().getPage(), worldName, rank))
                rankName = rank;

            // Make sure there is a default for the board
            if (!Settings.doesRankHaveScoreBoard(InfoBoard.getTimers()
                    .getPage(), worldName, rankName))
                return false;
                    ...
                    if (line.equals(" ") || line.equals("")) {
                        ...
                        if (line.startsWith("<scroll>")) {
                            ...
                            if (Settings.scrollingEnabled()) {
                                ...
                                Scroll sc = ScrollManager.createScroller(player,
                                        string, -row, longestLine);
                                line = sc.getMessage();
                                board.add(line, row);
                            } else {
                                line = "Enable Scroll";
                        ...
                        // If the line has a split in it
                            ...
                            try {
                                board.add(Messages.getLine(a, player), Integer
                                        .valueOf(Messages.getLine(b, player)));
